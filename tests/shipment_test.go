...
func (c *ClientTests) TestShipmentLowestRateDeprecated() {
	...
	// Test lowest rate with service filter (this rate is higher than the lowest but should filter)
	lowestRate, err = client.LowestRateWithCarrierAndService(shipment, nil, []string{"Priority"})
	require.NoError(err)
	assert.Equal("Priority", lowestRate.Service)
	assert.Equal("7.90", lowestRate.Rate)
	assert.Equal("USPS", lowestRate.Carrier)
	...
	// Test lowest rate with carrier filter (should error due to bad carrier)
	lowestRate, err = client.LowestRateWithCarrier(shipment, []string{"BAD_CARRIER"})
	assert.Error(err)
}
