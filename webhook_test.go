...
func (c *ClientTests) TestWebhookCreate() {
	...
	webhook, err := client.CreateWebhook(
		&CreateUpdateWebhookOptions{
			...
			CustomHeaders: c.fixture.WebhookCustomHeaders(),
		},
	)
	require.NoError(err)

	assert.Equal(reflect.TypeOf(&Webhook{}), reflect.TypeOf(webhook))
	assert.True(strings.HasPrefix(webhook.ID, "hook_"))
	assert.Equal(c.fixture.WebhookUrl(), webhook.URL)
	assert.Equal(c.fixture.WebhookCustomHeaders(), webhook.CustomHeaders)
...
func (c *ClientTests) TestWebhookRetrieve() {
	...
	assert.Equal(reflect.TypeOf(&Webhook{}), reflect.TypeOf(retrievedWebhook))
...
func (c *ClientTests) TestWebhookUpdate() {
	...
	updatedWebhook, err := client.UpdateWebhook(
		webhook.ID,
		&CreateUpdateWebhookOptions{
			WebhookSecret: c.fixture.WebhookSecret(),
			CustomHeaders: c.fixture.WebhookCustomHeaders(),
	...
	assert.Equal(reflect.TypeOf(&Webhook{}), reflect.TypeOf(updatedWebhook))
	assert.Equal(c.fixture.WebhookCustomHeaders(), updatedWebhook.CustomHeaders)
...
func (c *ClientTests) TestValidateWebhook() {
	...
	assert.Equal(webhookBody.Result.(*Tracker).Weight, 614.4) // Ensure we convert floats properly
}
